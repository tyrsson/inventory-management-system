...
final class Migration010Ticket implements MigrationInterface
{
    ...
    public function up(AdapterInterface $adapter): void
    {
        ...
        $create->addColumn(new Enum('ticketType', ['Delivery', 'Pickup']));

        $create->addColumn(
            (new Varchar('celerantRef', 100, nullable: true))
                ->setOptions(['comment' => 'Celerant order / ticket reference'])
        );

        $create->addColumn(new Varchar('customerName', 200));
        $create->addColumn(new Datetime('scheduledAt', nullable: true));
        ...
        $create->addConstraint(new Index(['storeId', 'status'], 'idx_ticket_store_status'));
        $create->addConstraint(new ForeignKey('fk_ticket_store', 'storeId', 'store', 'store_number'));
        $create->addConstraint(new ForeignKey('fk_ticket_completed_by', 'completedBy', 'user', 'id'));
        $create->addConstraint(new ForeignKey('fk_ticket_created_by', 'createdBy', 'user', 'id'));
